...
class Synchronized {
public:
  explicit Synchronized(T initial = T()) : _value(initial) {} //??

  struct Access {
    T& ref_to_value;
    mutex& am;
    Access(T& v, mutex& m) : ref_to_value(v), am(m) { am.lock(); }
    ~Access() { am.unlock(); }
  };

  Access GetAccess() {
    // lock_guard access_lock(_access_mutex);
    Access a(_value, _access_mutex);
    return a;
  }
...
vector<int> Consume(Synchronized<deque<int>>& common_queue) {
  vector<int> got; // возвращаемый объект
